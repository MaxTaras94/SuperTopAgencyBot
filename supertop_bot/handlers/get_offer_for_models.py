...
async def send_jo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> ConversationHandler:
    ...
    context.user_data[f"models_data"] = models_data
...
async def accept_jo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        await context.bot.send_message(chat_id=chat_id, text=render_template("mess_for_mng_when_model_accept_jo.j2", {"id_model_in_table":data_of_models}))
    ...
    if len(links_potrfolio_photo) <=10:
        ...
        await context.bot.send_message(chat_id=cast(Chat, chat_id), text=caption_text)
        await context.bot.send_media_group(chat_id=cast(Chat, chat_id), media=media_group, protect_content=False)
        return ConversationHandler.END
    else:
